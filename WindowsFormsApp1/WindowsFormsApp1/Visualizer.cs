...
namespace WindowsFormsApp1
{
    ...
    class Visualizer : GameWindow
    {
        ...
        protected override void OnRenderFrame(FrameEventArgs e)
        {
            ...
            Matrix4 modelview = Matrix4.LookAt(new Vector3(-25+x, -25+y,-4), new Vector3(2+x,2+y,15), -Vector3.UnitZ); //eye: -12,-12,-4//target:2,2,10
        ...
        private void DrawMap()
        {
            ...
            for (int x = 0; x < mapSize; x++)
            {
                for (int y = 0; y < mapSize; y++)
                {
                    for (int z = 0; z < 4; z++)
                    {
                        if (!Map[x, y, z].passable && !(Path[Path.Count-1].x == x && Path[Path.Count-1].y == y && Path[Path.Count-1].z == z))
                        {
        ...
        private void DrawPath()
        {
            ...
            for (int i = 1; i < Path.Count; i++)
            {
                ...
                if (i == 1)
                    color = new Vector3(0.05f, 0.9f, 0.1f);
                if (i == Path.Count - 1)
                    color = new Vector3(0.9f, 0.05f, 0.1f);
                DrawBox(0.5f, new Vector3(Path[i].x, Path[i].y, -Path[i].z - 0.5f), color);
            }
        }
